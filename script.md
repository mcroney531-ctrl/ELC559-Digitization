...
- **Scene rendering:** the procedure scene uses a rendered photo backdrop (`assets/scene-desk.jpg`, the Media Digitization Desk room with VCR/BrightBox/monitor) with the interactive/animated parts (tape counter, VCR button hotspots, cable-confirm outlets, capture LED, monitor software window, tape label, claim ticket) redrawn as CSS/HTML overlays aligned on top via a fixed mapping (canvas_x = photo_x·1.289, canvas_y = photo_y·1.289 − 190). Spotlighted equipment "punches through" the dim veil by re-showing its own region of the photo.
